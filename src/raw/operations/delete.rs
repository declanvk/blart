...
>(
    ...
    DeleteParentNodeChange<N::Key, N::Value, PREFIX_LEN>,
) {
    ...
    if inner_node.header().num_children() == 1 {
        ...
        (
            Some(child_node_ptr),
            DeleteParentNodeChange::SingletonCompressed { child_node_ptr },
        )
        ...
        (
            ...
            DeleteParentNodeChange::Shrunk {
                new_parent_node: new_inner_node_ptr,
            },
        )
    } else {
        (None, DeleteParentNodeChange::NoChange)
    }
}
...
pub enum DeleteParentNodeChange<K, V, const PREFIX_LEN: usize> {
...
impl<K, V, const PREFIX_LEN: usize> fmt::Debug for DeleteParentNodeChange<K, V, PREFIX_LEN> {
