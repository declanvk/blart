use crate::{
    ...
    raw::{
        ...
        InnerNode, InnerNode4, LeafNode, NodePtr, OpaqueNodePtr, PrefixMatch,
    },
...
impl<K, V, const PREFIX_LEN: usize> OverwritePoint<K, V, PREFIX_LEN> {
        ...
        A: Allocator,
    {
        ...
        match parent_ptr_and_child_key_byte {
            Some((parent_ptr, key)) => {
                // SAFETY: covered by doc comment
                unsafe { parent_write_child(parent_ptr, key, overwrite_ptr.to_opaque()) };
                ForceInsertResult {
                    insert_result: InsertResult {
