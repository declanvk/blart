...
use crate::{AsBytes, InnerNode256, InnerNode48, InnerNodeCompressed, OpaqueNodePtr, RestrictedNodeIndex};
...
pub struct InnerNodeCompressedIter<K: AsBytes, V> {
...
impl<K: AsBytes, V> InnerNodeCompressedIter<K, V> {
    ...
    pub unsafe fn range<const SIZE: usize, R: RangeBounds<u8>>(
        ...
        fn modify_iter_start<const SIZE: usize, K: AsBytes, V>(
        ...
        fn modify_iter_end<const SIZE: usize, K: AsBytes, V>(
            iter: &mut InnerNodeCompressedIter<K, V>,
            node: &InnerNodeCompressed<K, V, SIZE>,
            end_bound: Bound<&u8>,
        ) {
...
impl<K: AsBytes, V> Iterator for InnerNodeCompressedIter<K, V> {
...
impl<K: AsBytes, V> DoubleEndedIterator for InnerNodeCompressedIter<K, V> {
...
impl<K: AsBytes, V> FusedIterator for InnerNodeCompressedIter<K, V> {}

/// An iterator over all the children of a [`InnerNode48`].
// All the `NonNull` pointers in this struct are constructed from shared
// references and must not be used to mutate.
pub struct InnerNode48Iter<K: AsBytes, V> {
...
impl<K: AsBytes, V> InnerNode48Iter<K, V> {
    ...
    pub unsafe fn range<R: RangeBounds<u8>>(node: &InnerNode48<K, V>, range: R) -> Self {
        fn modify_iter_start<K: AsBytes, V>(iter: &mut InnerNode48Iter<K, V>, start_bound: Bound<&u8>) {
            let start_key_index = match start_bound {
        ...
        fn modify_iter_end<K: AsBytes, V>(iter: &mut InnerNode48Iter<K, V>, end_bound: Bound<&u8>) {
            let end_key_index = match end_bound {
...
impl<K: AsBytes, V> Iterator for InnerNode48Iter<K, V> {
    type Item = (u8, OpaqueNodePtr<K, V>);
...
impl<K: AsBytes, V> DoubleEndedIterator for InnerNode48Iter<K, V> {
    fn next_back(&mut self) -> Option<Self::Item> {
        while !self.child_indices_range.is_empty() {
            let next_ptr = {
                // SAFETY: Because we checked the loop condition, we know there is at least 1
...
impl<K: AsBytes, V> FusedIterator for InnerNode48Iter<K, V> {}

/// An iterator over all the children of a [`InnerNode256`].
// All the `NonNull` pointers in this struct are constructed from shared
// references and must not be used to mutate.
pub struct InnerNode256Iter<K: AsBytes, V> {
...
impl<K: AsBytes, V> InnerNode256Iter<K, V> {
    /// Create a new iterator over the children of the given [`InnerNode48`].
    ///
    /// # Safety
    ///
    ...
    pub unsafe fn range<R: RangeBounds<u8>>(node: &InnerNode256<K, V>, range: R) -> Self {
        fn modify_iter_start<K: AsBytes, V>(iter: &mut InnerNode256Iter<K, V>, start_bound: Bound<&u8>) {
            let start_key_index = match start_bound {
                Bound::Included(min_key_byte) => (*min_key_byte) as usize,
                Bound::Excluded(min_key_byte_excluded) => {
                    // We add 1 so that the value at index `min_key_byte_excluded` will be excluded.
        ...
        fn modify_iter_end<K: AsBytes, V>(iter: &mut InnerNode256Iter<K, V>, end_bound: Bound<&u8>) {
            let end_key_index = match end_bound {
                Bound::Included(min_key_byte) => {
                    // Since we want to include the value, we add one so the pointer will be one
                    // past the end of iteration
...
impl<K: AsBytes, V> Iterator for InnerNode256Iter<K, V> {
    type Item = (u8, OpaqueNodePtr<K, V>);
...
impl<K: AsBytes, V> DoubleEndedIterator for InnerNode256Iter<K, V> {
    fn next_back(&mut self) -> Option<Self::Item> {
        while !self.child_pointers_range.is_empty() {
            let next_ptr = {
                // SAFETY: Because we checked the loop condition, we know there is at least 1
...
impl<K: AsBytes, V> FusedIterator for InnerNode256Iter<K, V> {}
