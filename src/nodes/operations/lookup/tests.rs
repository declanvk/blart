use crate::{
    ...
    InnerNode48, LeafNode, VariableKeyHeader,
};

#[test]
fn lookup_on_non_copy_leaf() {
    let mut l1: LeafNode<Box<[u8]>, String, 16, VariableKeyHeader<16>> =
        LeafNode::new(Box::from([1, 2, 3]), "Hello world my name is".into());
    let mut l2: LeafNode<Box<[u8]>, String, 16, VariableKeyHeader<16>> =
...
#[test]
fn lookup_on_leaf() {
    let mut leaf: LeafNode<Box<[u8]>, i32, 16, VariableKeyHeader<16>> =
        LeafNode::new(Box::from([1, 2, 3]), 123);
...
fn lookup_on_n16_n4_layer_tree() {
    ...
    let mut l1: LeafNode<Box<[u8]>, i32, 16, VariableKeyHeader<16>> =
        LeafNode::new(Box::from([1, 2, 3, 5, 6, 1]), 123561);
    let mut l2: LeafNode<Box<[u8]>, i32, 16, VariableKeyHeader<16>> =
        LeafNode::new(Box::from([1, 2, 3, 5, 6, 2]), 123562);
    let mut l3: LeafNode<Box<[u8]>, i32, 16, VariableKeyHeader<16>> =
        LeafNode::new(Box::from([1, 2, 4, 7, 8, 3]), 124783);
    let mut l4: LeafNode<Box<[u8]>, i32, 16, VariableKeyHeader<16>> =
        LeafNode::new(Box::from([1, 2, 4, 7, 8, 4]), 124784);
