...
/// Removes a key from the tree, returning the LeafNode corresponding to the key
/// if the key was previously in the tree.
///
/// # Safety
///
...
pub unsafe fn delete_unchecked<V>(root: OpaqueNodePtr<V>, key: &[u8]) -> Option<DeleteResult<V>> {
    ...
    ///
    /// # Panics
    ///  - Panics if for input [`NodeType::Leaf`]
    ...
    /// # Safety
    ///
    ///  - `inner_node_ptr` must be a unique pointer to the node and must not
    ...
    unsafe fn delete_non_root_unchecked<V>(
    ...
    let DeleteSearchResult {
        ...
        parent_node_ptr,
        leaf_node_ptr,
    } = unsafe { search_for_node_to_delete(root, key)? };
    ...
    match (parent_node_ptr, grandparent_node_ptr) {
        ...
        (Some(parent_node_ptr), grandparent_node_ptr) => unsafe {
            ...
            Some(delete_non_root_unchecked(
                leaf_node_ptr,
                parent_node_ptr,
                grandparent_node_ptr,
                root,
